...
- `src/main.cpp` — globals (UART, NVS, `slots[]`, cross-task atomic hand-off flags), FreeRTOS task entrypoints, `setup()`, `loop()`, and `serial_log_vprintf`.
...
- **`T_DEV_LIST.slots[].state`** (added 2.1.0, G008): optional `u8` per entry carrying `VernierAdapter::ConnState`'s numeric value — `0=IDLE, 1=REQUESTED, 2=CONNECTING, 3=READY`. Pre-2.1.0 hosts ignore the key. Hosts that read it can render a CONNECTING spinner without inferring state from the absence of T_DEVINFO. `connected:bool` (= state==READY) stays in the frame so the contract is purely additive. (A `FAILED` state was designed but elided during implementation — failure transitions straight to IDLE; reintroduce as value 4 at the end of the enum if a future host-UX needs a distinct failure signal.)
...
- **Boot handshake**: `T_HELLO` (identity + proto version) followed by `T_STATUS(true)` is sent **once per co-MCU session**, lazily on the first host command (`dispatchHostCommand`). It is NOT sent per connect — the host's `T_HELLO` handler resets its whole connection state (it reads T_HELLO as "peer rebooted"), so a per-connect T_HELLO wiped already-connected slots during multi-slot auto-reconnect. The per-connect path (`publishConnectResult`) emits only per-slot frames (T_DEVINFO / T_DEVSTATS / T_FIELDS) + the trailing T_DEV_LIST.
...
- **Cross-task shared state.** When a variable is written by one FreeRTOS task and read by another, pick the lightest primitive that documents the contract:
  ...
  - **Fill-then-flag publish pattern** (writer fills a buffer / array, then flips a flag the reader polls) → make the flag a `std::atomic<T>`; store it with `memory_order_release` after the data fills and load it with `memory_order_acquire` before reading the data. The release/acquire edge makes the fills visible without an explicit barrier. Used for the boot hand-off triple `startAutoConnect` / `foundSavedDevice` / `slotHasSavedDevice[]`: the data array is relaxed, the two gate flags carry release/acquire (see `main.cpp` + `cmdSetPeriod` / `autoConnectDevice` in `control-loop.cpp`). The legacy equivalent — `volatile` flag + a `__asm__ volatile("" ::: "memory")` compiler barrier — also works on single-core ESP32-C3 (the Xtensa `memw` fence is neither needed nor assemblable) but release/acquire atomics are preferred for new code: they document the ordering portably. The one remaining barrier of this kind is in `publishConnectResult`, ordering the `uart.send*` burst before the relaxed `_conn_state` READY store (a non-atomic-before-atomic publish where release on the setter would need a separate overload).
